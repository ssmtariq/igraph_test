...
/* Function to resize 'order' vector with the specified capacity and initialize each element with -1 */
void resizeOrderVector(igraph_vector_int_t *order, size_t chunk_size_bytes) {
    size_t current_size = igraph_vector_int_size(order);
    size_t new_size = current_size + (chunk_size_bytes / sizeof(int));
    IGRAPH_CHECK(igraph_vector_int_resize(order, new_size));
    // Initialize newly added elements with -1
...
igraph_integer_t elements_read_so_far = 0;

// Function to manage order vector population
void manageOrderVector(igraph_vector_int_t *order, igraph_integer_t index, igraph_integer_t total_elements) {
    // Check if the order vector is fully populated
    if (!order_fully_populated && index>=elements_read_so_far) {
        ...
        if (index >= 0 && index < total_elements) {
            // Check if the access index is populated
                ...
                if (elements_read_so_far < total_elements) {
                    // Resize and initialize the order vector chunk by chunk
                    printf("Resizing and initializing the order vector chunk by chunk\n");
                    resizeOrderVector(order, CHUNK_SIZE);

                    // Update the last read offset
                    elements_read_so_far += (CHUNK_SIZE / sizeof(int));
